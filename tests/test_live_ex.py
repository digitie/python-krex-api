...
@pytest.mark.live
async def test_live_data_ex_traffic_by_ic_parses_real_payload() -> None:
...
@pytest.mark.live
async def test_live_data_ex_traffic_route_accepts_valid_key_even_when_empty() -> None:
    async with _live_client() as client:
        page = await client.traffic.by_route(
            route_no="0010",
            time_unit=TimeUnit.HOUR,
            num_of_rows=1,
            page_no=1,
...
@pytest.mark.live
async def test_live_restarea_route_facilities_accepts_real_payload_shape() -> None:
    async with _live_client() as client:
        page = await client.restarea.route_facilities(num_of_rows=1, page_no=1)
...
@pytest.mark.live
async def test_live_restarea_fuel_prices_parse_won_prices() -> None:
    async with _live_client() as client:
        page = await client.restarea.fuel_prices(num_of_rows=1, page_no=1)
...
@pytest.mark.live
async def test_live_restarea_convenience_facilities_keeps_raw_payload() -> None:
    async with _live_client() as client:
        page = await client.restarea.convenience_facilities(num_of_rows=1, page_no=1)
